...
playlist_url = "http://www.youtube.com/playlist?list=PL7atuZxmT954bCkC062rKwXTvJtcqFB8i"

# Set these to specific numbers to limit the files downloaded, or set to None to download everything.

playlist_range_start = 107
playlist_range_end = None

# "source" will extract the audio as it stands
...
xml_base_url = "http://audio.cr.mez.im/"
xml_title = "Critical Role (Unofficial)"
xml_author = None  # if None, picks arbitrary author from one of the videos
xml_image_url = "http://audio.cr.mez.im/artwork.png"  # if None, picks arbitrary thumbnail URL from one of the videos
...
allowed_audio_formats = [".aac", ".m4a", ".mp3", ".opus"]
...
def refresh_playlist_data():
    ...
    args = arg_call + ["--get-filename", "-o", '%(id)s']
...
def refresh_download_batch_file():
    status_message(
        "Generating list of missing videos in " + downloaded_files_directory + " to " + download_urls_filename + ".")
...
def download_with_batch_file():
    ...
    args = arg_call + ["--rate-limit", download_rate_limit, "--write-info-json", "--console-title", "--extract-audio",
                       "--output", audio_file_output_format, "--default-search", "ytsearch", "--batch-file", download_urls_directory]
    ...
    if os.path.isdir(downloaded_files_directory):
        ...
        for filename in files:
            ...
            if extension in allowed_audio_formats:
                ...
                podcast_xml_file.write(
                    "\t\t\t<enclosure url=\"" + format_for_html(xml_base_url + filename, True) + "\"></enclosure>\n")
                ...
                if id in json_data:
                    json_date = datetime.datetime.strptime(json_data[id]["upload_date"].encode("ascii", "ignore"),
                                                           "%Y%m%d")
                    ...
                    podcast_xml_file.write(
                        "\t\t\t<itunes:author>" + format_for_html(json_data[id]["uploader"]) + "</itunes:author>\n")
                    podcast_xml_file.write("\t\t\t<itunes:image href=\"" + format_for_html(image_url, True) + "\" />\n")
                    podcast_xml_file.write("\t\t\t<itunes:summary>" + format_for_html(
                        json_data[id]["description"]) + "</itunes:summary>\n")
